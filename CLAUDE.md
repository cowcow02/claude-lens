...
Claude subprocess. The pipeline is deliberately layered so no component
...
prior saved reports          ──► listSavedReports() + getSavedReport()
                              ...
                              ┌─────────────────────────┐
(PeriodBundle + capsules +    │  claude -p              │   ReportData (structured JSON)
...
- **`fleetlens stats` and `/api/insights` share the same library functions** (from `@claude-lens/parser`). CLI output byte-equal to server output for the same range.
