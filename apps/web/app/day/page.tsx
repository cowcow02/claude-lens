...
import { DayIndex, type DayRow } from "@/components/day-index";
...
export default function DayIndexPage() {
  ...
  for (let i = 0; i < DAYS_BACK; i++) {
    ...
    const status: DayRow["status"] = entries.length === 0 ? "empty"
    ...
    rows.push({
      ...
      pr_count: prCount,
      status,
      outcome: digest?.outcome_day ?? null,
  ...
  return (
    <div style={{ maxWidth: 980, margin: "0 auto", padding: "28px 40px" }}>
      ...
      >
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 700, margin: 0 }}>Day</h1>
          <p style={{ fontSize: 13, color: "var(--af-text-secondary)", marginTop: 4 }}>
            Last {DAYS_BACK} days · Click a day to open its narrative + timeline.
          </p>
        </div>
      </header>
      <DayIndex rows={rows} aiEnabled={aiEnabled} />
    </div>
  );
}
