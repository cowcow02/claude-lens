...
**Ships:** Day-scoped `Entry` primitive; deterministic + LLM enrichment; background daemon worker; daily digest page + `/digest/[date]` route; `fleetlens entries` / `fleetlens digest day` CLI
...
**Phases in this doc:** Phase 1 (entry layer) + Phase 2 (day digest). Phase 3 (entry-aware surfaces) and Phase 4 (V2 insights report) specified separately.

---

## Overview

Fleetlens gets a new, persistent **perception layer**. The primitive is a **Entry** — a structured summary of one session's work on one local day. Entries are generated deterministically from JSONL plus an optional LLM enrichment pass, stored on disk, and consumed by every surface in the product. The first consumer is a new **daily digest** page that answers "what did I do yesterday, what went well, what hit friction."

The architectural pivot: Anthropic's `/insights` uses sessions as the unit of perception. Fleetlens uses **(session × local-day)**. A session that spans Mon→Wed becomes three distinct Entries. A day with four sessions aggregates into one day digest over four Entries. Sessions are identity; days are meaning.

## Vocabulary (locked)

Exactly two terms across the codebase and UI:

- **Entry** — atomic perception artifact scoped to `(session_id, local_day)`. Always exists once events occur. Carries deterministic fields (numbers, flags, first_user, top_tools) plus LLM-enriched fields (brief_summary, friction_detail, goal, outcome, satisfaction_signals). Stored at `~/.cclens/entries/{session_id}__{YYYY-MM-DD}.json`. **TypeScript type name: `Entry`.**
- **Digest** — narrative synthesis over a set of Entries. Scopes: `day`, `week`, `month`, `project`, `session`. Stored at `~/.cclens/digests/{scope}/{key}.json`. Each digest schema differs by scope but shares a common envelope (`window`, `narrative`, `headline`, `generated_at`, `entry_refs`).

"Slice" is not used. "Facet" is not used externally. The existing **`SessionCapsule`** in `packages/parser/src/capsule.ts` is a distinct type for the legacy per-session shape used by V1 insights. `Entry` and `SessionCapsule` coexist in the codebase until Phase 4, when V2 insights retires `SessionCapsule`.
...
2. **Timeline already treats day as primary.** The timeline page lists sessions by day and always has. Entry scope matches what the UI already assumes.
3. **Anthropic structurally can't copy this.** Their `/insights` filters sessions into a window. To do day-scoped entries they'd have to rewrite the facet layer. Shipping this first establishes a moat on the qualitative-analysis axis.
...
The new `Entry` and V1 `SessionCapsule` share no type. `/insights` page behavior is byte-for-byte unchanged after Phase 1+2 ships. Phase 4 replaces V1 with V2 and only then deprecates `SessionCapsule`.
...
type Entry = {
  ...
  source_checkpoint: {                         // provenance only — daemon uses perception-state.json instead
    byte_offset: number                        // last offset parsed when this Entry was built
    last_event_ts: string | null               // last event timestamp seen when this Entry was built
  }
}
```
...
**Project determination for an Entry.** Computed deterministically at build time:
...
3. The dominant canonicalized cwd is the Entry's project. Tiebreak by earliest event.
4. If no events carry cwd metadata (rare — conversation-only slice), inherit from `SessionDetail.projectName` canonicalized.

Entries NEVER split by project within a single (session, day). A session that legitimately spans projects produces one Entry per day attributed to the dominant project; cross-project detail stays in the raw JSONL.
...
type DayDigest = DigestEnvelope & {
  ...
  agent_min: number                            // sum of entry.numbers.active_min
...
Note: a `session` digest is a different *view* of the same Entries — aggregating all Entries sharing a `session_id`. It is not a fifth Entry type.
...
- **Atomic writes.** All writes to files under `~/.cclens/entries/` and `~/.cclens/digests/` use the pattern *write-to-temp + fsync + rename*. Readers never observe half-written JSON.
- **Entry file lifecycle.** A Entry file is written at most **twice**:
...
- **Missing entry references.** When a DayDigest's `entry_refs` includes an Entry file that cannot be read (deleted, corrupted), the digest builder filters the reference out, logs a warning, and proceeds. If zero Entries remain, the digest is not generated and the route returns 404 with a message.

## Entry-per-day case handling

| Case | Resolution |
|---|---|
| Single session, single day | One Entry. Enrichment characterizes the whole session. |
| Multiple sessions, single day | N Entries, same `local_day`. Day digest aggregates. |
| Single session, multiple days | N Entries with shared `session_id`, different `local_day`. A session-digest view (Phase 3) stitches them. |
| Multiple sessions, multiple days | M×N Entries. Each day digest reads that day's Entries independently. |
| Session spans midnight | Events before midnight go to that day's Entry; events at/after to the next. Each Entry's `active_min` reflects only its day's events. Trivial Entries dropped. |
| Trivial slice | `numbers.active_min < 1` AND `numbers.turn_count < 3` AND `numbers.tools_total === 0` → Entry stored with `enrichment.status = "skipped_trivial"`, no LLM call, immutable from first write. |
| Resumed session with old context | Only events on target day enter that day's Entry. Old history invisible — correct, because the user isn't doing that work today. |
| In-flight day (today, before midnight) | Today's Entries have `enrichment.status = "pending"` until daemon finalizes them AFTER the day closes. Today's digest (in-memory TTL) uses whatever enrichment state exists at request time; may show partial narrative. |
| In-flight session on a completed day | Yesterday's Entry for that session is finalizable once yesterday closes (boundary passed). Today's Entry for the same session is independent and stays pending until today closes. |
| Concurrency on a day | Per-session Entries are independent. `DayDigest.concurrency_peak = aggregateConcurrency(bursts, {start: startOfLocalDay(date), end: endOfLocalDay(date)}).peak`. |
| Session spans weeks | Entries split cleanly by `local_day`. Week boundary is a grouping concern, not a storage concern. |
| Session touches multiple projects (rare) | Entry records dominant project (see Project determination above). |
| Failed/incomplete LLM enrichment | Entry finalized with `enrichment.status = "error"` and `error` message set. Day digest uses deterministic aggregates and marks the slice in `narrative` as "couldn't summarize" only if the LLM is asked to produce narrative; otherwise silent. Retried on next daemon sweep up to 3 times with exponential backoff, then left permanently in `error`. |
| Deleted/corrupted JSONL | Entry already on disk continues to be used. If never generated, cannot be rebuilt — system skips. |
| Late-flush events past midnight | Daemon waits 30 minutes after local midnight before finalizing a day's Entries, guarding against late appends. If a late append arrives after finalization, the Entry is permanently stale — accepted tradeoff; rare in practice. |
| Schema version bump | Daemon detects `version !== CURRENT_SCHEMA_VERSION` on read, queues regeneration, atomically overwrites via write-rename. Documented as the ONLY permitted rewrite. Changes to the trivial threshold or goal-category taxonomy require a schema bump to take effect (they alone do not trigger regeneration of already-written `skipped_trivial` or `done` Entries). |
...
[parser.ts parseTranscript → SessionDetail]                 (existing)
        │
        ▼
[entryBuilder.groupByDay(sessionDetail) → Entry[]]      (NEW, deterministic)
        │   Initial write: enrichment.status = "pending" or "skipped_trivial"
        ▼
~/.cclens/entries/{session_id}__{day}.json
        │
        ▼   (daemon sweep; only for non-trivial, settled-day Entries)
[entryEnricher.enrich(entry) → enriched Entry]        (NEW, LLM, one call)
        │   Finalization write: enrichment.status = "done" or "error"
        ▼
~/.cclens/entries/{...}.json (replaced atomically)
        │
        ▼   (on digest request for past day, or daemon schedules after all day's entries finalized)
[dayDigestBuilder.build(entries_for_day) → DayDigest]      (NEW)
...
Cost target: ~$0.001 per Entry with Sonnet 4.6.

### Day digest synthesis

Deterministic aggregations computed from Entries (no LLM):
- `projects` — sum active_min by canonical project, compute share_pct
- `shipped` — `entries.flatMap(c => c.pr_titles.map(t => ({title: t, project: c.project, session_id: c.session_id})))`
- `top_flags` — count flag occurrences across Entries, top 5
- `top_goal_categories` — sum enrichment.goal_categories across non-pending Entries, top 5
...
- `agent_min` — `sum(entries.map(c => c.numbers.active_min))`

Single LLM call for narrative fields: input is deterministic aggregations + up to 12 Entry brief_summaries + top 6 friction_details + up to 10 aggregated user_instructions. Output: `headline`, `narrative`, `what_went_well`, `what_hit_friction`, `suggestion`.
...
1. **Rebuild deterministic Entries.** For each Claude Code JSONL file whose current size > last `perception-state.json` checkpoint for that file, tail-parse new events, recompute affected Entries (the current-day Entry for that session, plus the previous-day Entry if today is within 30 min of midnight). Write initial Entry files atomically.
2. **Queue enrichment candidates.** Any Entry with `enrichment.status ∈ {"pending", "error"}` (retry count < 3) AND whose `local_day` is settled (strictly before today in local TZ, with ≥ 30 min of no activity) AND not `"skipped_trivial"`.
3. **Consume queue sequentially** at up to 3 concurrent LLM calls. Each call respects the budget guard (see below). On success, finalize the Entry via atomic write-rename. On failure, increment retry count and leave for next sweep.
4. **Day digest scheduling.** When all non-trivial Entries for a day reach `enrichment.status ∈ {"done", "error"}`, queue a single LLM call to synthesize the DayDigest; write atomically to `digests/day/{YYYY-MM-DD}.json`.
...
The web server and the daemon are separate Node processes. Both issue LLM calls (web for today's digest generation + V1 insights; daemon for Entry enrichment and past-day digest synthesis).

**Budget file.** `~/.cclens/llm-spend.jsonl` — append-only, one JSON record per LLM call:

```json
{"ts":"2026-04-22T14:00:01Z","caller":"daemon","model":"claude-sonnet-4-6","input_tokens":1200,"output_tokens":480,"cost_usd":0.0108,"kind":"entry_enrich","ref":"{sid}__2026-04-21"}
```
...
```
packages/entries/                             ← NEW, Node-only
  src/
    index.ts                                   types only, safe for browser via subpath
    types.ts                                   Entry, DigestEnvelope, DayDigest
    build.ts                                   SessionDetail → Entry[] (deterministic, pure)
    ...
    enrich.ts                                  entry → LLM → enriched Entry (Node-only)
    digest-day.ts                              Entries → DayDigest (Node-only, calls LLM)
    prompts/
...
- `packages/entries` (the root index) exports types only. Safe to import from anywhere, including browser components.
- `packages/entries/fs` exports `storage.ts`. Node-only. Imported by `packages/cli/**` and `apps/web/lib/entries.ts` (server-only module).
- `packages/entries/node` exports `enrich.ts` and `digest-day.ts`. Node-only. Imported ONLY by `packages/cli/**` and `apps/web/lib/ai/digest-day-gen.ts` (server-only, used by the `/api/digest` route).
- Next.js is configured to treat `packages/entries/fs` and `packages/entries/node` as server-only via the `server-only` marker package imported at the top of each.

```
packages/parser/                               ← UNCHANGED in Phase 1+2
  src/entry.ts                               SessionCapsule — stays, used by V1 insights
  src/aggregate.ts                             PeriodBundle — stays, used by V1 insights

packages/cli/                                  ← Phase 1+2 additions
  src/
    commands/entries.ts                       fleetlens entries
...
apps/web/                                      ← Phase 2 additions
  ...
  lib/entries.ts                              server-only: reads storage via packages/entries/fs
  lib/ai/digest-day-gen.ts                     server-only: calls packages/entries/node for today's digest
...
- **Today (date === today in server's local TZ):** compute deterministic aggregations from current Entries; if `ai_features.enabled`, call LLM for narrative (or serve from in-memory 10-min TTL cache); return 200.
...
```
fleetlens entries [--day YYYY-MM-DD | --session UUID | --all] [--json]
fleetlens entries regenerate [--since YYYY-MM-DD] [--force]
fleetlens digest day [--date YYYY-MM-DD | --yesterday | --today] [--json | --pretty]
```

By default reads-only. The `entries regenerate` subcommand and synchronous digest generation for today/uncached past days trigger LLM work. Respects the same budget guard as the daemon.
...
- The README updates to "local-only dashboard with opt-in AI enrichment." The `/settings` page includes a transparency block listing exactly what's sent when enrichment is on (entry facts, up to 8 human turns per Entry, up to 12 summaries + 6 friction lines per day-digest call — never raw full transcripts, file contents, or tool result payloads).

## Schema versioning

- `CURRENT_ENTRY_SCHEMA_VERSION = 2` on first ship (bumped from the legacy SessionCapsule's conceptual v1 so they're distinguishable).
- `CURRENT_DAY_DIGEST_SCHEMA_VERSION = 2` on first ship.
- Schema bumps queue regeneration via the same daemon mechanism. Regeneration is the only permitted modification of a finalized Entry file. UI can safely render v2-only; no multi-version rendering needed.

## Testing strategy

**Unit (`packages/entries/test/`):**
- `build.test.ts` — fixture SessionDetail spanning midnight produces expected Entries keyed by (session, day) with correct `active_min` clipping; projection through canonicalProjectName; trivial-threshold behavior.
...
- `digest-day.test.ts` — deterministic aggregations exact-match for a fixture day; LLM-null-path renders placeholder fields; shipped list derivation matches entry.pr_titles.

**Integration:**
- CLI-vs-web parity: `fleetlens entries --day X --json | jq '.[0]'` byte-equal to the Entry loaded by `/api/digest/day/X` server code for the first Entry of the day.
...
- Deterministic Entry build: O(events), unchanged asymptotic from existing `buildCapsule`. A 1M-line JSONL ~1s.
- Entry files: 3–8 KB each. 1000 Entries ≈ 5 MB disk.
- Enrichment: Sonnet input ~2 KB, output ~500 B; ~$0.001/call. 50 Entries/day ≈ $0.05/day ≈ $1.50/month for a heavy user. A fleet-orchestration user with 5 concurrent agents × 3 days × 5 sessions = 75 Entries/day ≈ $2.25/month.
- Day digest: Sonnet input ~5 KB, output ~800 B; ~$0.002/call. 30 days ≈ $0.06/month.
- **Scales linearly in Entry count; heaviest realistic monthly cost ~$5** — well under the Claude Code Max plan's noise floor.

## Rollout

1. Land `packages/entries/` types + build + signals + trivial + storage. Ship with enrichment/digest code present but gated by `ai_features.enabled`. No UI changes.
...
5. Phase 3 (separate spec): integrate Entries into Timeline, Session Detail, Calendar, Projects pages.
6. Phase 4 (separate spec): V2 weekly/monthly insights report reading Entries. Only at this point do we retire `SessionCapsule` from `packages/parser/src/capsule.ts` (replaced by an Entry-based aggregator).
...
- Search / semantic retrieval over Entries — future.
- Trend detection / anomaly alerts — future.
- Hard budget enforcement (`enforce_hard_budget` flag) — future.
- Live (in-flight) Entry enrichment — enrichment only runs on settled days.
